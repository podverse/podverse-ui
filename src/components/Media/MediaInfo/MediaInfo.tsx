...
export class MediaInfo extends React.Component<Props, State> {
  ...
  render() {
    ...
    if (mediaRef) {
      ...
      createdByName =
        mediaRef.owner && mediaRef.owner.name
          ? mediaRef.owner.name
          : t("Anonymous");
    ...
    return (
      <React.Fragment>
        ...
        <div className="media-info">
          {episodeTitle && (
            <Link as={episodeAs} href={episodeHref}>
              <a
          ...
          {episodePubDate && (
            <div className="media-info__episode-pub-date">{episodePubDate}</div>
          )}
          {clipTitle && (
            <React.Fragment>
              <div className="media-info__clip-title">{clipTitle}</div>
            </React.Fragment>
          )}
          {clipTime && (
            <div className="media-info__clip-time">
              <a className="media-info__replay-clip" onClick={handleReplayClip}>
                {isOfficialChapter
            ...
            !currentItem.podcastHideDynamicAdsWarning && (
              <div className="media-info__dynamic-ads-warning">
                <Trans i18n={i18n} i18nKey="NoteDynamicAds">
                  Note: If a podcast uses{" "}
                  <DynamicAdsWarningLink>
                    {t("dynamic ads")}
                  </DynamicAdsWarningLink>
                  , the clip start time may not stay accurate.
                </Trans>
              </div>
            )}
          {(episode || mediaRef || nowPlayingItem) && moreInfo && (
            ...
            >
              <span>
                <FontAwesomeIcon
                  icon={showDescription ? "caret-down" : "caret-right"}
                />
                &nbsp;{t("Episode Notes")}
              </span>
            </button>
...
const removeClipStartEventListeners = () => {
  if (eventListenerElements.length > 0) {
    for (const eventListenerElement of eventListenerElements) {
      eventListenerElement.removeEventListener(
        "click",
        setClipStartEventListeners
      );
    }
